...
Use the following values for your ${chalk.blue(
...
${chalk.green(`PSOXY_${prefix}_CLIENT_SECRET`)}: ${data.clientSecret}
  ...
  const notesTmpl = `
${chalk.bold('NOTE:')} the secret/parameter names above are examples only, 
and may vary based on your configuration, such as if you specified  some 
environment name/id, or SSM parameter path in your ${chalk.blue(
    'terraform.tfvars'
  )} file.
  `;
  console.log(`\n${notesTmpl.trim()}`);
